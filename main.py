    ...
    for i, (location, theta_i, phi_i) in enumerate(light_positions):
        ...
        if light_type == 'AREA':
            ...
            if shape == 'RECTANGLE':
                ...
                light.size = cm_to_m(dim_a_cm) # Major Axis A
...
# input: configuration, output path, film exposure, loop parameters
# output: renders the scene for each light source sequentially
def render(cfg, output_path, film_exposure, loop_params):
    ...
    for i, light in enumerate(lights_to_render):
        ...
        # Naming: <source number>_<exposure value used>
        filename = f"{i+1:03d}_{film_exposure:.3f}.png"
...
def main():
    ...
    # 1. Prepare Exposure Values
    exp_cfg = cfg.get('exposure')
    exposure_start = exp_cfg['start']
    exposure_end = exp_cfg['end']
    exposure_step = exp_cfg['step']
    num_steps = int((exposure_end - exposure_start) / exposure_step) + 1
    exposure_values = [round(exposure_start + i * exposure_step, 3) for i in range(num_steps)]
    ...
    for p_config in plane_configs:
        ...
        # Create output folder for Plane Config
        plane_dir_name = (
        ...
        plane_output_path = os.path.join(cfg.base_output_dir, plane_dir_name)
        os.makedirs(plane_output_path, exist_ok=True)
