...
class UnifiedMasker:
    ...
    def process(self):
        image_files = [f for f in self.input_dir.iterdir() if f.suffix.lower() in {".jpg", ".cr2", ".png", ".exr", ".nef"}]
    ...
    def run_ai_mask(self, img_8bit):
        ...
        masks, _, _ = self.predictor.predict(point_coords=np.array([[w//2, h//2]]), point_labels=np.array([1]), multimask_output=False)
...
if __name__ == "__main__":
    UnifiedMasker(r"C:\Users\vishn\Desktop\avanthik\nrml_mp_cd\unified_masker_cfg.json").process()