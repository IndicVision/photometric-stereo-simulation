...
class AutoIterativePipeline:
    ...
    def save_visualizations(self, iter_num, normals, df, Z, dx, dy, mask):
        iter_dir = self.output_dir / f"iteration_{iter_num:02d}"; iter_dir.mkdir(exist_ok=True)
        u_idx, v_idx = df['pixel_u'].values.astype(int), df['pixel_v'].values.astype(int)
        h, w = self.cfg['resolution']['height'], self.cfg['resolution']['width']
        n_map = np.zeros((h, w, 3), dtype=np.float32); n_map[v_idx, u_idx] = normals
        cv2.imwrite(str(iter_dir / "normal_map.png"), cv2.cvtColor(((n_map+1)/2*65535).astype(np.uint16), cv2.COLOR_RGB2BGR))
        r, c = np.where(mask); Zc = Z[np.min(r):np.max(r)+1, np.min(c):np.max(c)+1]*1000
        plt.imshow(Zc); plt.colorbar(); plt.savefig(iter_dir / "2D_depth_map.png"); plt.close()
        df_out = df.copy(); df_out['z_world'] = Z[v_idx, u_idx]
