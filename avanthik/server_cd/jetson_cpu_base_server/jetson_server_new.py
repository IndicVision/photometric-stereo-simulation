...
# ─────────────────────────────────────────────────────────────────────────────
def esp32_get(path):
    url = "http://{}:{}{}".format(ESP32_IP, ESP32_PORT, path)
    try:
        # Send the request and wait up to 1.5 seconds for the HTTP response
...
def handle_ws_message(text, conn):
    ...
    if text.startswith("SET_PREVIEW:"):
        # Example text: "SET_PREVIEW:1,3"
        light_ids = text.split(":")[1] 
        # We will create this endpoint in the ESP32 C code next!
    ...
    # We can remove the old PREVIEW_LIGHT_ON since we are using SET_PREVIEW now
    else:
...
def run_pipeline_thread(png_files, json_files, save_dir):
    try:
        ...
        from process_pipeline_new import run_pipeline
...
class PhotoStereoHandler(BaseHTTPRequestHandler):
    ...
    def handle_websocket(self):
        ...
        esp32_get("/all_off") # <-- ADD THIS LINE to guarantee lights turn off
    ...
    # ── Result — phone downloads ZIP when status == done ──────────────────────
    def handle_result(self):
        job = get_job()
        if job["status"] != "done":
            self._json(409, {"error": "not ready", "status": job["status"]}); return
            
        # Pull the path (still stored under the "html" key for now) but name it correctly
