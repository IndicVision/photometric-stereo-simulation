...
FLAT_PLANE_ERROR = False   # Compute mean angular error vs flat plane [0,0,1]
...
class AutoIterativePipeline:
    ...
    def _get_memory_usage(self):
        """Returns current CPU RAM and GPU VRAM usage in MBs."""
        # CPU RAM used by this specific Python process
        process = psutil.Process(os.getpid())
        ram_mb = process.memory_info().rss / (1024 * 1024)
        
        # GPU VRAM managed by CuPy's memory pool
        vram_mb = cp.get_default_memory_pool().used_bytes() / (1024 * 1024)
        
        # Total System Swap (shows if we are running out of physical RAM)
    ...
    def __init__(self, config_input):
        ...
        # --- ADD THIS: Memory profiling dictionary ---
        # We will store [Peak RAM (MB), Peak VRAM (MB), Peak Swap (MB)]
        self.mem_stats = {
    ...
    def estimate_normals(self, df):
        ...
        # --- MODIFIED: CPU memory bounce hidden behind debug flags ---
        if DEBUG:
        ...
        # ONLY cast to CPU memory if the Flat Plane Error flag is True
        if FLAT_PLANE_ERROR:
            if not DEBUG: normals_cpu = cp.asnumpy(normals) # Catch if DEBUG is false
            self._flat_plane_error(normals_cpu, label="estimated | pre-Poisson")

        del G, G_w, GT_w, GTG, GTI, n_est
        cp.get_default_memory_pool().free_all_blocks()
        
        # --- MODIFIED: Return raw CuPy array! ---
        return normals

    # -------------------------------------------------------------------------
    def repair_normals_gpu(self, normals_gpu, v_idx_gpu, u_idx_gpu, M_img, N_img):
        """
        OPTIMIZED: 100% GPU execution using cupyx. No CuPy <-> NumPy transfers.
        """
        ...
        # Use float32 to save Jetson Nano RAM
        ...
        normals_repaired = cp.stack([
            nx_g[v_idx_gpu, u_idx_gpu],
            ny_g[v_idx_gpu, u_idx_gpu],
            nz_g[v_idx_gpu, u_idx_gpu]
        ], axis=1)

        # Free GPU memory explicitly
        del nx_g, ny_g, nz_g, mask_g, good_g, bad_g, good_f, nx_sum, ny_sum, nz_sum, w_sum
        cp.get_default_memory_pool().free_all_blocks()
    ...
    def reconstruct_surface(self, df, normals):
        ...
        if not hasattr(self, '_recon_cache'):
            ...
            # --- REVERTED: Must be float64 (default) to reach 1e-10 tolerance ---
            ...
            # ── Build AMG hierarchy once — replaces SuperLU factorization ────────
            # PyAMG smoothed aggregation on C = AᵀA (symmetric positive definite).
            # Builds a V-cycle hierarchy that solves global shape AND local detail.
            # Memory: stores only the sparse hierarchy — no LU fill-in.
            # Time: ~1-5 seconds to build vs 485 seconds for SuperLU factorization.
            ...
            self._dbp("SOLVER", f"AMG hierarchy built ✓  |  time: {t_factor:.3f} s  |  levels: {len(ml.levels)}")
            if DEBUG:
                print(f"  [DBG|SOLVER      ] AMG hierarchy: {ml}")
            ...
            self._recon_cache = dict(
                ...
                # --- MODIFIED: Add CuPy variables for GPU gradient math ---
                keep_gpu=cp.array(keep),
                u_idx_e_gpu=cp.array(u_idx_e),
                v_idx_e_gpu=cp.array(v_idx_e),
                mask_gpu=cp.array(mask),
                r_H_gpu=cp.array(r_H), c_H_gpu=cp.array(c_H),
                r_V_gpu=cp.array(r_V), c_V_gpu=cp.array(c_V)
            )
        ...
        # --- MODIFIED: Use GPU cache arrays ---
        normals_e_gpu = normals[rc['keep_gpu']]

        # Debug prints every iteration
        ...
        # ── Normal grids from current iteration's normals (ON GPU) ────────────
        nx_grid = cp.zeros((M_img, N_img), dtype=cp.float32)
        ny_grid = cp.zeros((M_img, N_img), dtype=cp.float32)
        nz_grid = cp.zeros((M_img, N_img), dtype=cp.float32)
        
        nx_grid[rc['v_idx_e_gpu'], rc['u_idx_e_gpu']] = normals_e_gpu[:, 0]
        ny_grid[rc['v_idx_e_gpu'], rc['u_idx_e_gpu']] = normals_e_gpu[:, 1]
        nz_grid[rc['v_idx_e_gpu'], rc['u_idx_e_gpu']] = normals_e_gpu[:, 2]   # raw, no abs()

        if DEBUG:
            nz_on_mask   = cp.asnumpy(nz_grid[rc['mask_gpu']])
            ...
            for comp_name, comp_grid_gpu in [("nx", nx_grid), ("ny", ny_grid), ("nz", nz_grid)]:
                v = cp.asnumpy(comp_grid_gpu[rc['mask_gpu']])
            ...
            print(f"  [DBG|RECONSTRUCT]   |nz| < 0.05   : {pct_nz_small:.1f}%")

        # ── Gradients p, q (COMPUTED ON GPU) ──────────────────────────────────
        eps_nz = cp.float32(0.15)
        valid_gpu  = rc['mask_gpu']
        
        nz_abs_clamped = cp.maximum(cp.abs(nz_grid), eps_nz)
        nz_safe = cp.where(nz_grid >= 0, nz_abs_clamped, -nz_abs_clamped)
        nz_safe[~valid_gpu] = 1.0

        p = cp.zeros((M_img, N_img), dtype=cp.float32)
        q = cp.zeros((M_img, N_img), dtype=cp.float32)
        p[valid_gpu] = -(nx_grid[valid_gpu] / nz_safe[valid_gpu]) * cp.float32(dx)
        q[valid_gpu] = +(ny_grid[valid_gpu] / nz_safe[valid_gpu]) * cp.float32(dy)

        if DEBUG:
            pv = cp.asnumpy(p[valid_gpu])
            qv = cp.asnumpy(q[valid_gpu])
        ...
        # ── Build RHS b from current gradients (ON GPU) ───────────────────────
        val_p = p[rc['r_H_gpu'], rc['c_H_gpu']]
        val_q = q[rc['r_V_gpu'], rc['c_V_gpu']]
        
        # --- REVERTED: Transfer 1D vector to CPU and cast to float64 for PyAMG ---
        b_gpu = cp.concatenate([val_p, val_q, cp.array([0.0], dtype=cp.float32)])
        b = cp.asnumpy(b_gpu).astype(np.float64)

        self._dbp("SOLVER", f"RHS vector b: ||b||₂ = {np.linalg.norm(b):.4f}")
        if DEBUG:
            print(f"  [DBG|SOLVER      ] Equation weights: OLS (uniform = 1.0)")

        # ── Solve: d = Aᵀb, z = PyAMG V-cycle (CG accelerated by AMG) ──────
        # accel='cg': uses AMG as preconditioner for CG.
        # This gives BOTH global shape accuracy (from multigrid V-cycles)
        # AND local detail sharpness (from CG fine-grid correction).
        # tol=1e-10: tight tolerance — ensures no blurriness from early stopping.
        d = A_T @ b
        t_solve = time.time()
        z = ml.solve(d, tol=1e-8, accel='cg')
        t_solve = time.time() - t_solve

        if DEBUG:
            self._dbp("SOLVER", f"AMG solve complete ✓  |  time: {t_solve:.3f} s")
            residual     = A @ z - b
    ...
    # -------------------------------------------------------------------------
    def run(self):
        """OPTIMIZED: GPU normal repair and End-of-Pipeline visual generation."""
        print("\n" + "="*65)
        print("  AREA-LIGHT PS Pipeline (COMBINED)  [DEBUG={}, FLAT_PLANE_ERROR={}]".format(DEBUG, FLAT_PLANE_ERROR))
        print("="*65)

        t_pipeline_start = time.time() # START GLOBAL TIMER
        ...
        def record_mem(step_name):
            """Updates the peak memory for a given step."""
        ...
        # Variables to hold the final state for visualization
        ...
        for i in range(self.max_iterations):
            ...
            # ---- Normal Repair (OPTIMIZED TO GPU) ----
            print(f"\n  > Repairing Normals...")
            t_rep = time.time()
            u_idx_r = cp.array(df['pixel_u'].values.astype(int))
            v_idx_r = cp.array(df['pixel_v'].values.astype(int))
            M_img_r = int(cp.max(v_idx_r).item()) + 1
            N_img_r = int(cp.max(u_idx_r).item()) + 1
            
            # --- MODIFIED: normals is already a CuPy array, no cp.array() needed ---
            normals = self.repair_normals_gpu(normals, v_idx_r, u_idx_r, M_img_r, N_img_r)
            ...
            # Store states in case this is the last iteration
            final_normals, final_Z_map, final_dx, final_dy, final_mask = normals, Z_map, dx, dy, mask
            final_iter_num = i

            # ---- Update df and Convergence ----
            t_conv = time.time()
        ...
        # ---- Save Visualizations (ONLY ONCE AT THE END) ----
        print(f"\n  > Saving FINAL Visualizations...")
        t_vis = time.time()
        
        # --- MODIFIED: Cast final_normals back to NumPy for saving ---
        final_normals_cpu = cp.asnumpy(final_normals)
        current_csv, current_z_vals = self.save_visualizations(final_iter_num, final_normals_cpu, df, final_Z_map, final_dx, final_dy, final_mask)
        ...
        # --- END OF PIPELINE: PRINT PROFILING RESULTS ---
