...
class AutoIterativePipeline:
    ...
    def repair_normals_gpu(self, normals_gpu, v_idx_gpu, u_idx_gpu, M_img, N_img):
        ...
        return cp.asnumpy(normals_repaired)
    ...
    def reconstruct_surface(self, df, normals):
        ...
        if not hasattr(self, '_recon_cache'):
            ...
            # --- MODIFIED: Force float32 to save RAM ---
            V_list = np.concatenate([
                np.ones(num_H, dtype=np.float32), -np.ones(num_H, dtype=np.float32),
                np.ones(num_V, dtype=np.float32), -np.ones(num_V, dtype=np.float32),
                np.array([1.0], dtype=np.float32)
            ])
            A = coo_matrix((V_list, (I_list, J_list)), shape=(num_eq, num_unknowns), dtype=np.float32).tocsr()
        ...
        # --- MODIFIED: Force b to float32 ---
        b = np.concatenate([val_p, val_q, [0.0]]).astype(np.float32)
    ...
    def run(self):
        ...
        for i in range(self.max_iterations):
            ...
            normals_gpu = cp.array(normals)
            normals = self.repair_normals_gpu(normals_gpu, v_idx_r, u_idx_r, M_img_r, N_img_r)
