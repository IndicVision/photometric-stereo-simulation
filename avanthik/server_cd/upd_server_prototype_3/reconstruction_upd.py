...
class AutoIterativePipeline:
    ...
    def reconstruct_surface(self, df, normals_np):
        ...
        # ── Floor anchor only — detrending is deferred to save_visualizations ──
        # No detrending is applied here. Raw Poisson Z is returned so that
        # z_world updates for subsequent iterations are not biased by the
        # quadratic fit, and convergence is measured on the true Poisson output.
        ...
        if DEBUG:
            ...
            self._dbp("DETREND", f"Raw Z (no detrend): min={Z_valid_post.min():.4f} mm  "
                                  f"max={Z_valid_post.max():.4f} mm  "
                                  f"mean={Z_valid_post.mean():.4f} mm  "
                                  f"std={Z_valid_post.std():.4f} mm  "
                                  f"range={np.ptp(Z_valid_post):.4f} mm")

        return Z, rc['dx'], rc['dy'], rc['mask']

    # -------------------------------------------------------------------------
    def _apply_detrend(self, Z_raw, mask):
        """Apply detrending to a floor-anchored raw Z map.

        The detrending mode is read from config key
        global_settings.detrending_mode (default: 'quadratic').
        Supported values: 'quadratic', 'linear', 'none'.

        Returns Z_det (new array, Z_raw is never modified). If mode is 'none',
        returns a plain copy of Z_raw. Otherwise re-floors so minimum is 0.
        """
        mode = self.cfg.get('global_settings', {}).get('detrending_mode', 'quadratic').lower()
        rc   = self._recon_cache

        valid_X     = rc['X_grid'][mask]
        valid_Y     = rc['Y_grid'][mask]
        valid_Z     = Z_raw[mask]
        Z_before_mm = np.ptp(valid_Z) * 1000.0
        Z_det       = Z_raw.copy()

        if mode == 'quadratic':
            self._dbp("DETREND", "Mode = QUADRATIC — removing macro bowl/tilt (final output stage)")
            A_fit = np.c_[valid_X**2, valid_Y**2, valid_X * valid_Y,
                          valid_X, valid_Y, np.ones_like(valid_X)]
            C_fit, _, _, _ = np.linalg.lstsq(A_fit, valid_Z, rcond=None)
            Detrend_surface = (C_fit[0] * rc['X_grid']**2 + C_fit[1] * rc['Y_grid']**2 +
                               C_fit[2] * rc['X_grid'] * rc['Y_grid'] + C_fit[3] * rc['X_grid'] +
                               C_fit[4] * rc['Y_grid'] + C_fit[5])
            Z_det[mask] = Z_det[mask] - Detrend_surface[mask]
            self._dbp("DETREND", f"  Coefficients: a={C_fit[0]:.4e}  b={C_fit[1]:.4e}  "
                                  f"c={C_fit[2]:.4e}  d={C_fit[3]:.4e}  e={C_fit[4]:.4e}  f={C_fit[5]:.4e}")

        elif mode == 'linear':
            self._dbp("DETREND", "Mode = LINEAR — removing planar tilt only (final output stage)")
            A_fit = np.c_[valid_X, valid_Y, np.ones_like(valid_X)]
            C_fit, _, _, _ = np.linalg.lstsq(A_fit, valid_Z, rcond=None)
            Detrend_surface = (C_fit[0] * rc['X_grid'] + C_fit[1] * rc['Y_grid'] + C_fit[2])
            Z_det[mask] = Z_det[mask] - Detrend_surface[mask]
            self._dbp("DETREND", f"  Plane coefficients: nx={C_fit[0]:.4e}  ny={C_fit[1]:.4e}  d={C_fit[2]:.4e}")

        else:
            self._dbp("DETREND", "Mode = NONE — no detrend applied at final output stage")
            return Z_det   # already floor-anchored from reconstruct_surface, skip re-floor

        # Re-floor so minimum stays at 0 after the detrend shift
        floor = np.nanmin(Z_det[mask])
        Z_det[mask] = Z_det[mask] - floor
        Z_after_mm  = np.ptp(Z_det[mask]) * 1000.0
        self._dbp("DETREND", f"  Z range before: {Z_before_mm:.4f} mm  ->  after: {Z_after_mm:.4f} mm")

        return Z_det

    # -------------------------------------------------------------------------
    def _save_one_surface(self, iter_dir, tag, label, iter_num,
                          Z, dx, dy, mask, normals_np, df):
        """Write 2-D depth map PNG, interactive 3-D HTML, and CSV for one Z map.

        tag   — short file-suffix, e.g. 'detrended' or 'raw'
        label — human-readable string used in plot titles
        """
        u_idx = df['pixel_u'].values.astype(int)
        v_idx = df['pixel_v'].values.astype(int)
        ...
        im = ax.imshow(Z_crop_mm,
                       extent=[x_plot_mm[0], x_plot_mm[-1], y_plot_mm[-1], y_plot_mm[0]],
                       cmap='viridis')
        ax.set_title(f"2D Depth Map ({label}) — Iteration {iter_num}",
                     fontsize=13, fontweight='bold')
        ax.set_xlabel('X (mm)')
        ax.set_ylabel('Y (mm)')
        fig.colorbar(im, ax=ax, label='Depth (mm)', fraction=0.046, pad=0.04)
        plt.tight_layout()
        plt.savefig(iter_dir / f"2D_depth_map_{tag}.png", dpi=200)
        plt.close(fig)

        # ── 3-D interactive surface ───────────────────────────────────────────
        z_valid_for_color = Z_crop_mm[~np.isnan(Z_crop_mm)]
        ...
        fig3d = go.Figure(data=[go.Surface(
            z=Z_crop_mm, x=x_plot_mm, y=y_plot_mm,
        ...
        fig3d.update_layout(
            title=f"3D Surface ({label}) — Iteration {iter_num}  [Z ×{Z_EXAGGERATION:.1f}]",
            scene=dict(
        ...
        html_path = iter_dir / f"3D_surface_interactive_{tag}.html"
        ...
        csv_path = iter_dir / f"mapping_iter{iter_num}_{tag}.csv"
        df_out.to_csv(csv_path, index=False)

        self._dbp("VIS", f"[{label}] 2D PNG + 3D HTML + CSV saved  (tag='{tag}')")
        return html_path, csv_path, z_vals

    # -------------------------------------------------------------------------
    def save_visualizations(self, iter_num, normals_np, df, Z, dx, dy, mask):
        """Save all final outputs.

        Z is the raw floor-anchored Poisson result (no detrend).
        This method applies the quadratic detrend internally and produces
        two complete sets of outputs — one detrended, one raw.
        """
        ...
        # ── Normal map (shared, independent of detrend) ───────────────────────
        h_res = self.cfg['resolution']['height']
        w_res = self.cfg['resolution']['width']
        ...
        cv2.imwrite(str(iter_dir / "normal_map.png"),
                    cv2.cvtColor(n_vis_uint16, cv2.COLOR_RGB2BGR))

        # ── Compute detrended Z ───────────────────────────────────────────────
        detrend_mode = self.cfg.get('global_settings', {}).get('detrending_mode', 'quadratic').lower()
        detrend_label = {'quadratic': 'Quadratic Detrended',
                         'linear':    'Linear Detrended',
                         'none':      'No Detrend'}.get(detrend_mode, 'Detrended')
        Z_det = self._apply_detrend(Z, mask)

        # ── Flat plane error for both surfaces ────────────────────────────────
        rc = self._recon_cache
        self._flat_plane_error_from_Z(Z_det, mask, rc['v_idx_e'], rc['u_idx_e'],
                                      dx, dy, label="reconstructed | post-Poisson+detrend")
        self._flat_plane_error_from_Z(Z, mask, rc['v_idx_e'], rc['u_idx_e'],
                                      dx, dy, label="reconstructed | post-Poisson raw")

        # ── Save detrended outputs ────────────────────────────────────────────
        html_det, csv_det, z_vals_det = self._save_one_surface(
            iter_dir, tag='detrended', label=detrend_label,
            iter_num=iter_num, Z=Z_det, dx=dx, dy=dy, mask=mask,
            normals_np=normals_np, df=df)

        # ── Save raw outputs ──────────────────────────────────────────────────
        html_raw, csv_raw, z_vals_raw = self._save_one_surface(
            iter_dir, tag='raw', label='Raw (No Detrend)',
            iter_num=iter_num, Z=Z, dx=dx, dy=dy, mask=mask,
            normals_np=normals_np, df=df)

        self._dbp("VIS", f"Plotly surface grid: "
                         f"{Z_det[np.min(np.where(mask)[0]):np.max(np.where(mask)[0])+1, :].shape[0]}"
                         f"×"
                         f"{Z_det[:, np.min(np.where(mask)[1]):np.max(np.where(mask)[1])+1].shape[1]}"
                         f" pts (full resolution)")
        self._dbp("VIS", f"Z exaggeration factor (from JSON 'z_exaggeration'): "
                         f"{float(self.cfg.get('global_settings', {}).get('z_exaggeration', 1.0)):.1f}×  "
                         f"(Detrended Z range: {np.ptp(Z_det[mask])*1000:.4f} mm  |  "
                         f"Raw Z range: {np.ptp(Z[mask])*1000:.4f} mm)")

        # Return the detrended html path and raw z_vals for backward-compat
        # with the run() return value (html path) and any callers expecting z_vals
        return csv_det, Z[v_idx, u_idx]
    ...
    def run(self):
        ...
        html_candidate = self.output_dir / f"iteration_{final_iter_num:02d}" / "3D_surface_interactive_detrended.html"
        if html_candidate.exists():
            return str(html_candidate)
        # Fallback to raw if somehow detrended didn't write
        html_fallback = self.output_dir / f"iteration_{final_iter_num:02d}" / "3D_surface_interactive_raw.html"
