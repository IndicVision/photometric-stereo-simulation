...
class HybridMaskerU2NetP:
    ...
    def process(self):
        ...
        # This gives the traditional CV room to find the exact, crisp physical edge.
        ...
        # Run Otsu just like in the trad_cv script to get perfect edges
        ...
        # Keep ONLY the precise traditional edges that fall inside the AI's Safe Zone
        hybrid_mask = cv2.bitwise_and(trad_cv_mask, safe_zone)

        print("Step 6: Morphological Cleanup & Edge Smoothing...")
        # 1. Use ELLIPTICAL kernels to close internal holes
        ...
        # 2. Guided Filter Edge Polish (Smart Anti-Aliasing)
        # We slightly blur the mask so the filter has a gradient to push around
        mask_for_guide = cv2.GaussianBlur(hybrid_mask, (11, 11), 0)
        
        try:
            # Guide it using the original high-res grayscale image
            refined_soft_mask = cv2.ximgproc.guidedFilter(guide=gray, src=mask_for_guide, radius=15, eps=0.01)
            # Threshold it back to a crisp binary mask
            _, hybrid_mask = cv2.threshold(refined_soft_mask, 127, 255, cv2.THRESH_BINARY)
            print(" -> Guided Filter applied successfully.")
        except AttributeError:
            print(" -> ERROR: cv2.ximgproc not found! Falling back to standard Gaussian melt.")
            smoothed_mask = cv2.GaussianBlur(hybrid_mask, (15, 15), 0)
            _, hybrid_mask = cv2.threshold(smoothed_mask, 127, 255, cv2.THRESH_BINARY)
        
        # 3. Keep only the largest contour to ensure no isolated noise remains
...
if __name__ == "__main__":
    ...
    # Listen to server.py first. Only use the hardcoded path if running manually!
    config_file = sys.argv[1] if len(sys.argv) > 1 else r"C:\Users\vishn\Desktop\avanthik\nrml_mp_cd\mask_mapp\mbl_msk_test\trad_guidedFilter_u2netp_msk_cfg.json"
    
    if os.path.exists(config_file):
        processor = HybridMaskerU2NetP(config_file)
        processor.process()
    else:
