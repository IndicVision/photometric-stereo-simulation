...
class UniversalConverter:
    ...
    def run(self):
        print(f"{'='*40}\nSTARTING BATCH CONVERSION (LOSSLESS PIPELINE)\n{'='*40}")
        
        # Fetch the list of images
        input_paths = self.cfg['io_settings'].get('input_paths', [])
        
        for path in input_paths:
            print(f"\nProcessing: {path}")
            try:
                self._load_image(path)  # Pass the path here
                self._process_image()
                self._save_image(path)  # Pass the path here to grab its filename
    ...
    def _load_image(self, path): # <-- Add 'path' parameter
        # Remove the old line that pulled the single path from self.cfg
    ...
    def _process_image(self):
        ...
        if cm['mode'] == 'srgb_to_linear':
            ...
            if self.is_linear_data:
                ...
                print("Status: Gamma is 1.0. Skipping linearization math to save compute.")
                self.is_linear_data = True # Tag it as linear
            else:
        ...
        if cm['mode'] == 'linear_to_srgb':
            if gamma == 1.0:
                print("Status: Gamma is 1.0. Skipping encoding math to preserve linear state.")
                # We intentionally DO NOT set self.is_linear_data = False here!
            else:
    ...
    def _save_image(self, path): # <-- Add 'path' parameter
        ...
        # Dynamically grab the original filename without the extension
        base_filename = os.path.splitext(os.path.basename(path))[0]
        
        # Construct the new output path
        out_path = os.path.join(io_set['output_folder'], f"{base_filename}{tgt['extension']}")
...
if __name__ == "__main__":
    converter = UniversalConverter(r"C:\Users\vishn\Desktop\avanthik\cmprsn_cd\img_stdy\upd_convert_img_cfg.json")
    converter.run()