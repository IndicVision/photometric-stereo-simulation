...
def print_all_exif_data(file_path):
    print(f"\n{'-'*30}")
    print("FULL EXIF METADATA")
    print(f"{'-'*30}")
    try:
        with open(file_path, 'rb') as f:
            tags = exifread.process_file(f)
            if not tags:
                ...
                for tag, value in tags.items():
                    # Skip massive binary dumps for thumbnails to keep output readable
                    if tag in ('JPEGThumbnail', 'TIFFThumbnail', 'EXIF MakerNote'):
                        print(f"{tag:<35}: [Binary Data]")
                    else:
...
image_path = r"C:\Users\vishn\Downloads\light_001_1.jpg"

if os.path.exists(image_path):
    get_image_details(image_path)
    check_orientation(image_path)
    print_all_exif_data(image_path)
    print("\n")
else:
